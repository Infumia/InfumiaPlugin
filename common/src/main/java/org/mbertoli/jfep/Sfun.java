...
public class Sfun {
   ...
   */
  static public double acosh(final double x) {
    final double ans;
  ...
  static public double asinh(final double x) {
    ...
    if (Double.isNaN(x)) {
      ...
      ans = x * (1.0 + Sfun.csevl(2.0 * x * x - 1.0, Sfun.ASINH_COEF));
  ...
  static public double atanh(final double x) {
    ...
    if (Double.isNaN(x)) {
      ...
      ans = x * (1.0 + Sfun.csevl(8.0 * x * x - 1.0, Sfun.ATANH_COEF));
    } else if (y < 1.0) {
  ...
  static public double cosh(final double x) {
    final double ans;
    final double y = Math.exp(Math.abs(x));
    if (Double.isNaN(x)) {
      ans = Double.NaN;
  ...
  static public double cot(final double x) {
    ...
    double y;
    final double yrem;
    final double pi2rec = 0.011619772367581343075535053490057; //  2/PI - 0.625
    y = Math.abs(x);
    if (y > 4.5036e+15) {
    ...
    if (y == 0.0) {
      ...
      ans = (0.5 + Sfun.csevl(32.0 * y * y - 1.0, Sfun.COT_COEF)) / y;
    } else if (y <= 0.5) {
      ans = (0.5 + Sfun.csevl(8.0 * y * y - 1.0, Sfun.COT_COEF)) / (0.5 * y);
      ans = (ans * ans - 1.0) * 0.5 / ans;
    } else {
      ans = (0.5 + Sfun.csevl(2.0 * y * y - 1.0, Sfun.COT_COEF)) / (0.25 * y);
      ans = (ans * ans - 1.0) * 0.5 / ans;
      ans = (ans * ans - 1.0) * 0.5 / ans;
    }
    if (x != 0.0) {
   ...
   */
  static public double erf(final double x) {
    final double ans;
    final double y = Math.abs(x);
    if (y <= 1.49012e-08) {
   ...
   */
  static public double erfc(final double x) {
    double ans;
    final double y = Math.abs(x);
    if (x <= -6.013687357) {
      // -6.013687357 = -Math.sqrt(-Math.getLog(1.77245385090551602729816748334 * EPSILON_SMALL))
      ...
      if (y < 1) {
        ans = 1 - x * (1 + Sfun.csevl(2 * ysq - 1, Sfun.ERFC_COEF));
      } else if (y <= 4.0) {
        ans = Math.exp(-ysq) / y * (0.5 + Sfun.csevl((8.0 / ysq - 5.0) / 3.0, Sfun.ERFC2_COEF));
        if (x < 0) {
          ans = 2.0 - ans;
        }
        if (x < 0) {
        ...
        ans = Math.exp(-ysq) / y * (0.5 + Sfun.csevl(8.0 / ysq - 1, Sfun.ERFCC_COEF));
        if (x < 0) {
          ans = 2.0 - ans;
        }
      }
   ...
   */
  static public double fact(final int n) {
    double ans = 1;
   ...
   */
  static public double gamma(final double x) {
    double ans;
    double y = Math.abs(x);
    if (y <= 10.0) {
      /*
      ...
      y = x - n;
      n--;
      ans = 0.9375 + Sfun.csevl(2.0 * y - 1.0, Sfun.GAMMA_COEF);
      if (n == 0) {
      } else if (n < 0) {
      ...
      if (x > 171.614) {
        ...
        ans = Math.exp((y - 0.5) * Math.log(y) - y + 0.9189385332046727 + Sfun.r9lgmc(y));
        if (x < 0.0) {
          final double sinpiy = Math.sin(Math.PI * y);
   ...
   */
  static public double logBeta(final double a, final double b) {
    final double corr;
    final double ans;
    final double p = Math.min(a, b);
    final double q = Math.max(a, b);
    if (p <= 0.0) {
      ans = Double.NaN;
    } else if (p >= 10.0) {
      // P and Q are large;
      corr = Sfun.r9lgmc(p) + Sfun.r9lgmc(q) - Sfun.r9lgmc(p + q);
      final double temp = Sfun.dlnrel(-p / (p + q));
      ans = -0.5 * Math.log(q) + 0.918938533204672741780329736406 + corr + (p - 0.5) * Math.log(p / (p + q)) + q * temp;
    } else if (q >= 10.0) {
      // P is small, but Q is large
      corr = Sfun.r9lgmc(q) - Sfun.r9lgmc(p + q);
      //  Check from underflow from r9lgmc
      ans = Sfun.logGamma(p) + corr + p - p * Math.log(p + q) + (q - 0.5) * Sfun.dlnrel(-p / (p + q));
    } else {
      // P and Q are small;
      ans = Math.log(Sfun.gamma(p) * (Sfun.gamma(q) / Sfun.gamma(p + q)));
    }
    return ans;
  }
   ...
   *   If x is a negative integer, the result is NaN.
   */
  static public double logGamma(final double x) {
    final double ans;
    final double sinpiy;
    final double y;
    y = Math.abs(x);
    if (y <= 10) {
      ans = Math.log(Math.abs(Sfun.gamma(x)));
    } else if (x > 0) {
      // A&S 6.1.40
      // 0.9189385332046727 = 0.5*getLog(2*PI)
      ans = 0.9189385332046727 + (x - 0.5) * Math.log(x) - x + Sfun.r9lgmc(y);
    } else {
      ...
      if (sinpiy == 0 || Math.round(y) == y) {
        ...
        ans = 0.22579135264472743236 + (x - 0.5) * Math.log(y) - x - Math.log(sinpiy) - Sfun.r9lgmc(y);
      }
    }
    return ans;
  }
  ...
  static public double sinh(final double x) {
    ...
    if (Double.isNaN(x)) {
      ...
      ans = x * (1.0 + Sfun.csevl(2.0 * x * x - 1.0, Sfun.SINH_COEF));
    } else {
      y = Math.exp(y);
      if (y >= 94906265.62) {
        // 94906265.62 = 1.0/Math.sqrt(EPSILON_SMALL)
        ans = Sfun.sign(0.5 * y, x);
      } else {
        ans = Sfun.sign(0.5 * (y - 1.0 / y), x);
      }
    }
    return ans;
  }
  ...
  static public double tanh(final double x) {
    ...
    if (Double.isNaN(x)) {
      ...
      ans = x * (1.0 + Sfun.csevl(2.0 * x * x - 1.0, Sfun.TANH_COEF));
   ...
   */
  static double csevl(final double x, final double[] coef) {
    double b0;
    double b1;
    double b2;
    final double twox;
   ...
   */
  static double r9lgmc(final double x) {
    final double ans;
    if (x < 10.0) {
      ans = Double.NaN;
    } else if (x < 9.490626562e+07) {
      // 9.490626562e+07 = 1/Math.sqrt(EPSILON_SMALL)
      final double y = 10.0 / x;
      ans = Sfun.csevl(2.0 * y * y - 1.0, Sfun.R9LGMC_COEF) / x;
   ...
   */
  private static double dlnrel(final double x) {
    final double ans;
    if (x <= -1.0) {
      ans = Double.NaN;
    } else if (Math.abs(x) <= 0.375) {
      ans = x * (1.0 - x * Sfun.csevl(x / .375, Sfun.ALNRCS_COEF));
    } else {
      ans = Math.log(1.0 + x);
    }
    return ans;
   ...
   */
  static private double sign(final double x, final double y) {
