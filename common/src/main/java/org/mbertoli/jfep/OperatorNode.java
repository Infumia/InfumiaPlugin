...
public class OperatorNode implements ExpressionNode {
   ...
   */
  public OperatorNode(final ExpressionNode left, final ExpressionNode right, final char operation) {
  ...
  @Override
  public Object clone() {
    final ExpressionNode n_left = (ExpressionNode) this.left.clone();
    final ExpressionNode n_right = (ExpressionNode) this.right.clone();
   ...
   */
  private boolean needBrackets(final ExpressionNode child, final boolean isleft) {
    final char childSubtype = child.getSubtype().charAt(0);
